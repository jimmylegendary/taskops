...
taskops promote-partials ./my-work --apply --json
```
...
| `max_loopbacks`        | The `--max-loopbacks` budget is exhausted; a pending self-delegate is still open.                        |
...
`taskops restart <work-dir> --from <task-id> --instruction "<text>"` rolls the active version of the task's task group forward to a new version that re-executes from the named task, while preserving completed upstream work and superseding (not deleting) the prior version.

```bash
# re-do task-target with new context; downstream tasks reset to pending, upstream stays done
taskops restart ./my-work --from task-target \
  --instruction "Re-do with the new acceptance criteria from review." \
  --reason post_review_change --json

# alternatively, read the instruction body from a file
taskops restart ./my-work --from task-target --instruction-file ./restart-note.md
```

Restart semantics:

- The new version copies tasks ordered by `task.order` from the active selected version that contains `<task-id>`.
