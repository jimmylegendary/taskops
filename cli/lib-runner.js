...
function buildAgentLoopbackPrompt({ project, delegate, runId, loopbackNodeId, artifactRelPath }) {
  return [
    'You are a TaskOps self-loopback resolution agent.',
    `Work: ${project.id} — ${project.title || ''}`.trim(),
    `Work objective: ${project.objective || ''}`,
    '',
    ...
    '',
    'The delegation target is the runner itself (self). Take the work back and produce a concrete resolution that lets downstream execution continue.',
...
function performDryRunLoopback({ runDir, loopbackNodeId, delegate }) {
  const now = isoNow();
  const artifactsDir = join(runDir, 'artifacts');
  ensureDir(artifactsDir);
  const artifactPath = join(artifactsDir, `${loopbackNodeId}.md`);
  const lines = [
    `# Self-loopback resolution artifact for ${delegate.id}`,
    '',
    ...
    '',
    '## Loopback outcome',
    '- The runner took the work back deterministically and closed the delegate.',
...
function performAgentLoopback({ project, projectDir, delegate, agentId, stepTimeoutMs, runDir, runId, loopbackNodeId }) {
  const artifactsDir = join(runDir, 'artifacts');
  ensureDir(artifactsDir);
  const artifactPath = join(artifactsDir, `${loopbackNodeId}.md`);
  const artifactRelPath = artifactPath.startsWith(projectDir) ? artifactPath.slice(projectDir.length).replace(/^[\\/]/, '') : artifactPath;
  const prompt = buildAgentLoopbackPrompt({ project, delegate, runId, loopbackNodeId, artifactRelPath });
  const args = ['agent', '--agent', agentId, '--message', prompt, '--json'];
  if (stepTimeoutMs != null && Number.isFinite(stepTimeoutMs)) {
    args.push('--timeout', String(Math.max(1, Math.floor(stepTimeoutMs / 1000))));
  }
...
function executeSelfLoopback({ projectDir, project, delegate, runDir, runId, eventsPath, executor, agentId, stepTimeoutMs, loopbackIndex }) {
  ...
  const loopbackPath = ensureRunNode({
    ...
    type: 'loopback',
    title: `Self-loopback resolution for ${delegate.id}`,
  ...
  writeRunEdge({
    ...
    edgeType: 'loopback',
    createdAt: startedAt,
    note: 'Self-loopback resolution opened by runner',
  });
  ...
  logEvent(eventsPath, {
    ...
    sourceTaskId: delegate.sourceTaskId || null,
    sourceTaskGroupVersionId: delegate.sourceTaskGroupVersionId || null,
    executor, loopbackIndex,
  });
  appendRunLog(runDir, `${startedAt} loopback_started delegateRunNodeId=${delegate.id} loopbackRunNodeId=${loopbackNodeId} executor=${executor}`);

  let result;
  try {
    result = executor === 'dry-run'
      ? performDryRunLoopback({ runDir, loopbackNodeId, delegate })
      : performAgentLoopback({ project, projectDir, delegate, agentId, stepTimeoutMs, runDir, runId, loopbackNodeId });
  } catch (err) {
  ...
  if (!result.ok) {
    ...
    return {
      kind: 'loopback', status: 'failed', executor,
  ...
  if (delegatePath && existsSync(delegatePath)) {
    rewriteFrontmatter(delegatePath, (fm) => {
      fm.status = 'done';
      fm.resolvedBy = 'self_loopback';
      fm.resolvedAt = finishedAt;
      fm.resolvedByRunNodeId = loopbackNodeId;
      return fm;
    });
  }
  closeRunNodeWithEow({ runDir, runId, runNodeId: delegate.id, reason: 'self_loopback_resolved', finishedAt });
  ...
  appendRunLog(runDir, `${finishedAt} loopback_completed delegateRunNodeId=${delegate.id} loopbackRunNodeId=${loopbackNodeId} artifact=${result.artifactPath || ''}`);

  return {
    kind: 'loopback', status: 'completed', executor,
    delegateRunNodeId: delegate.id, runNodeId: loopbackNodeId,
    artifactPath: result.artifactPath || null,
    message: result.message || null,
    loopbackIndex,
...
export function runTaskOps(workDir, options = {}) {
  ...
  try {
    ...
    while (true) {
      ...
      if (next.kind === 'stop') {
        if (
          ...
          if (delegate && isSelfDelegate(delegate, parsed.project)) {
            if (loopbacksUsed >= maxLoopbacks) {
              stopReason = STOP_REASONS.MAX_LOOPBACKS;
              stopDetail = `Self-loopback budget exhausted at ${loopbacksUsed}/${maxLoopbacks}; pending self delegate ${delegate.runId}/${delegate.id} still open.`;
    ...
    return {
      ...
      executor,
      loopbackPolicy, maxLoopbacks, loopbacksUsed,
      eventsPath,
      tasks: actions,
      actions,
    };
