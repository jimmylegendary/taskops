...

{
  ...
  const generatedTaskEowId = taskEowId({
    taskGroupVersionId: 'tgv-root-v1',
    taskId: 'task-a',
  });
  const taskEowPath = join(root, 'task-groups', 'tg-root', 'versions', 'tgv-root-v1', 'eow', `${generatedTaskEowId}.md`);
  assert.equal(
    readFileSync(taskEowPath, 'utf8'),
    fmBlock({
      taskOpsVersion: 'v1',
      entityType: 'eow',
      id: generatedTaskEowId,
      graphType: 'task',
      attachedToType: 'task',
      attachedToId: 'task-a',
      taskGroupVersionId: 'tgv-root-v1',
    ...
    'task EoW output should include its canonical identity when resolvedByTaskGroupId is omitted',
  );
  ...
  const generatedRunEowId = runEowId({
    runId: 'run-main',
    runNodeId: 'run-node-task-a',
  });
  const runEowPath = join(runDir, 'nodes', `${generatedRunEowId}.md`);
  assert.equal(
    readFileSync(runEowPath, 'utf8'),
    fmBlock({
      taskOpsVersion: 'v1',
      entityType: 'eow',
      id: generatedRunEowId,
      runId: 'run-main',
      graphType: 'run',
      attachedToType: 'runNode',
    ...
    'run EoW output should include its canonical identity when resolvedByTaskGroupId is omitted',
  );
